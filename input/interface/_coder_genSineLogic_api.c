/*
 * File: _coder_genSineLogic_api.c
 *
                      ...
                      const mxArray *plhs[4])
{
  ...
  emxInit_real_T(&st, &t);
  emxInit_real_T(&st, &x);
  genSineLogic(A, f, duration, Fs, t, x, &samples, &cycles);
...
void genSineLogic_atexit(void)
{
  ...
  genSineLogic_xil_terminate();
  genSineLogic_xil_shutdown();
  emlrtExitTimeCleanup(&emlrtContextGlobal);
}
...
/*
 * File trailer for _coder_genSineLogic_api.c
 *
 * [EOF]
 */
